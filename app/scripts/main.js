...
>>>>>>> 1bd57611d684a7e517af277dc0906d940ecc745b
...
function findHome(data){
  if(!data.referrer){
    ...
    var node = findNode(data.referrer,root)
    if(node){
      console.log(node)
      node.children.push({
        name:data.name,
...
function findNode(referrer, node){
  if(node.name == referrer){
    return node
  }

  for(var i = 0; i < node.children.length;i ++){
    var foundNode = findNode(referrer,node.children[i])
